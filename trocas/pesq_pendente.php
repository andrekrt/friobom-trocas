...
$stmt = $db->prepare("SELECT COUNT(*) AS allcount FROM troca WHERE analise_apoio = 'aguardando'");
$stmt->execute();
$records = $stmt->fetch();
$totalRecords = $records['allcount'];

## Total number of records with filtering
$stmt = $db->prepare("SELECT COUNT(*) AS allcount FROM troca WHERE 1 AND analise_apoio = 'aguardando' ".$searchQuery);
$stmt->execute($searchArray);
$records = $stmt->fetch();
$totalRecordwithFilter = $records['allcount'];

## Fetch records
$stmt = $db->prepare("SELECT * FROM troca WHERE 1 ".$searchQuery." AND analise_apoio = 'aguardando'  ORDER BY ".$columnName." ".$columnSortOrder." LIMIT :limit,:offset");
